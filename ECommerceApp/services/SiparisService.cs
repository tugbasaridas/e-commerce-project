        ...
        try
        {
            ...
            foreach (var sepetItem in sepetUrunleri)
            {
                ...
                siparisDetaylari.Add(new SiparisDetay
                {
                    ...
                    PlatformKomisyonu = komisyonTutari, // YENİ: Platformun payı
                    SaticiKazanci = saticiKazanci,      // YENİ: Satıcının hesabına yatacak tutar
                    Durum = "Hazırlanıyor"
                });
            }
   ...
   public async Task<object> SiparisGecmisiniGetirAsync(int userId)
    {
        return await _db.Siparisler
            ...
            .Select(s => new
            {
                ...
                {
                    DetayId = d.Id,
                    UrunId = d.UrunId,  // <--- İŞTE EKSİK OLAN VE 400 HATASI VERDİREN SATIR BURASI!
