...
export default function AdminUrunEkle() {
  ...
  const kaydet = async () => {
    ...
    try {
      const yeniUrun = {
        ad: ad,
        aciklama: aciklama || "Açıklama girilmedi.",
        fiyat: parseFloat(fiyat),
        stok: parseInt(stok, 10),
        resimUrl: resimUrl || "https://via.placeholder.com/150", 
        kategoriId: seciliKategoriId
      };

      await api.post('/urunler', yeniUrun);
  ...
  return (
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.formContainer} showsVerticalScrollIndicator={false}>
        ...
        {/* Resim URL */}
        <View style={styles.inputGrup}>
          <Text style={styles.label}>Resim URL (Opsiyonel)</Text>
          <TextInput 
            style={styles.input}
            placeholder="https://..."
            autoCapitalize="none"
        ...
        {/* Kaydet Butonu */}
        <TouchableOpacity style={styles.kaydetButon} onPress={kaydet} disabled={loading}>
          {loading ? (
