...
export default function SaticiUrunDuzenle() {
  const router = useRouter();
  const params = useLocalSearchParams();
  
  const [ad, setAd] = useState(params.ad as string || '');
  const [aciklama, setAciklama] = useState(params.aciklama as string || '');
  const [fiyat, setFiyat] = useState(params.fiyat?.toString() || '');
  const [stok, setStok] = useState(params.stok?.toString() || '');
  
  // GÜNCELLENDİ: Resim URL'si doğrudan params üzerinden otomatik alınıyor
  const [resimUrl, setResimUrl] = useState(params.resimUrl as string || '');
  
  const [seciliKategori, setSeciliKategori] = useState<number | null>(params.kategoriId ? Number(params.kategoriId) : null);
  ...
  useEffect(() => {
    const kategorileriGetirVeEsle = async () => {
      try {
        const response = await api.get('/kategori');
        const liste = response.data;
        setKategoriler(liste);

        // MEVCUT KATEGORİYİ BUL VE ADINI / AÇILMASINI SAĞLA
        if (params.kategoriId) {
          const hedefId = Number(params.kategoriId);
          
          for (const ana of liste) {
            // Eğer ürün doğrudan ana kategoriye aitse
            if (ana.id === hedefId) {
              setSeciliKategoriAdi(ana.ad);
              break;
            }
            // Eğer ürün bir alt kategoriye aitse
            if (ana.altKategoriler && ana.altKategoriler.length > 0) {
              const bulunanAlt = ana.altKategoriler.find((alt: any) => alt.id === hedefId);
              if (bulunanAlt) {
                setSeciliKategoriAdi(`${ana.ad} > ${bulunanAlt.ad}`);
                // Ürünün ait olduğu ana kategorinin akordeonunu otomatik AÇ
                setAcikKategoriler(prev => ({ ...prev, [ana.id]: true }));
                break;
              }
            }
          }
  ...
  const urunGuncelle = async () => {
    ...
    try {
      // Backend'e hem güncellenen metinsel alanlar hem de resimUrl gönderiliyor
      await api.put(`/satici/urun/${params.id}`, {
        ad: ad.trim(),
      ...
      setLoading(false);
      bildirimGoster("Ürün başarıyla güncellendi!", 'basari');
      ...
      setLoading(false);
      const hataMesaji = error.response?.data?.mesaj || error.response?.data?.Mesaj || error.response?.data || "Ürün güncellenirken bir hata oluştu.";
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.formGroup}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              ...
              {seciliKategoriAdi ? (
                <Text style={styles.secilenKategoriBilgi} numberOfLines={1}>Mevcut: {seciliKategoriAdi}</Text>
              ) : null}
            </View>
            ...
            {kategoriLoading ? (
               <ActivityIndicator size="small" color="#007AFF" style={{ alignSelf: 'flex-start', marginVertical: 10 }} />
            ) : (
              <View style={{ gap: 8, marginTop: 8 }}>
                {filtrelenmisKategoriler.length === 0 ? (
                  <Text style={styles.bosAramaText}>Kategori bulunamadı.</Text>
                ) : (
                        ...
                        <View style={styles.anaKategoriSatir}>
                          ...
                          >
                            <Ionicons 
                              ...
                              size={18} 
                              color={seciliKategori === anaKat.id ? "#007AFF" : "#8E8E93"} 
                              style={{ marginRight: 8 }}
                            />
                            <Text style={[styles.anaKategoriBaslik, seciliKategori === anaKat.id && { color: '#007AFF' }]}>
                              📁 {anaKat.ad}
                            </Text>
                          </TouchableOpacity>
                          ...
                          {hasSub && (
                            ...
                            >
                              <Text style={styles.altSayiBadge}>{anaKat.altKategoriler.length} alt</Text>
                              <Ionicons name={isOpen ? "chevron-up" : "chevron-down"} size={16} color="#8E8E93" />
                            </TouchableOpacity>
                          )}
                        </View>
                        
                        {/* Alt Kategoriler (Akordeon) */}
                        {isOpen && hasSub && (
                            ...
                            {anaKat.altKategoriler.map((altKat: any) => (
                              ...
                              >
                                <Ionicons 
                                  ...
                                  size={15} 
                                  color={seciliKategori === altKat.id ? "#007AFF" : "#8E8E93"} 
                                  style={{ marginRight: 6 }}
                                />
          ...
          <TouchableOpacity style={styles.kaydetButon} onPress={urunGuncelle} disabled={loading}>
            {loading ? (
              <ActivityIndicator color="#fff" />
            ) : (
              <>
                <Ionicons name="checkmark-done" size={22} color="#fff" style={{ marginRight: 8 }} />
                <Text style={styles.kaydetButonYazi}>Değişiklikleri Kaydet</Text>
              </>
            )}
          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  secilenKategoriBilgi: { fontSize: 12, fontWeight: '600', color: '#007AFF', maxWidth: '55%' },
  ...
  kategoriChipAktif: { backgroundColor: '#E5F1FF', borderColor: '#007AFF' },
  kategoriYazi: { fontSize: 12, color: '#48484A', fontWeight: '500' },
  kategoriYaziAktif: { color: '#007AFF', fontWeight: 'bold' },
  ...
  kaydetButon: { flexDirection: 'row', backgroundColor: '#007AFF', padding: 16, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginTop: 10, shadowColor: '#007AFF', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 4 },
