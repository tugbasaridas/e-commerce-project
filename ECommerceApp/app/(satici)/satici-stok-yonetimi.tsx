...
export default function SaticiStokYonetimi() {
  ...
  const urunleriGetir = async () => {
    try {
      const response = await api.get('/satici/urunlerim');
      
      // ÇÖZÜM BURADA: Sadece admin onayından geçmiş ve aktif olan ürünleri filtreliyoruz
      ...
      onayliUrunler.forEach((urun: Urun) => {
  ...
  const stokKaydet = async (id: number) => {
    ...
    setIslemdekiId(id);
    try {
      // DTO oluşturmak için ürünün mevcut bilgilerini çekelim.
      ...
      const guncelUrunData = {
        ...
        resimUrl: mevcutUrunDetay.resimUrl || null
      };

      await api.put(`/satici/urun/${id}`, guncelUrunData);
      ...
      Alert.alert("Hata", error.response?.data?.mesaj || "Stok güncellenemedi.");
    } finally {
      setIslemdekiId(null);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <TouchableOpacity onPress={() => router.navigate('/(satici)/satici-anasayfa' as any)}>
